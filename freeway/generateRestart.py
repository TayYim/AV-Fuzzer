...
def generateRestart(ck_path, scenarioNum, bounds):
    ...
    for i in range(scenarioNum):
        chromosome = Chromosome(bounds, npcSize, len(
            prevPopPool[0][0].scenario[0]))
    ...
    for i in range(scenarioNum):
        ...
        for j in range(popPoolSize):
            ...
            for k in range(scenarioSize):
                ...
                simi = tools.getSimilaritybetweenScenarios(
                    scenario1, scenario2)
                simiPop += simi
...
def IsDifferenceEnough(ck_path, afterScenario, beforeScenario):
    ...
    for j in range(popPoolSize):
        ...
        for k in range(scenarioSize):
            ...
            simiBefore = tools.getSimilaritybetweenScenarios(
                beforeScenario.scenario, referScenario)
            simiAfter = tools.getSimilaritybetweenScenarios(
                afterScenario.scenario, referScenario)
...
if __name__ == '__main__':
    ...
    npcSize = len(prevPopPool[0][0].scenario)
    chromosome1 = Chromosome([[0, 34], [0, 3]], npcSize,
                             len(prevPopPool[0][0].scenario[0]))
    chromosome1.rand_init()
    chromosome2 = Chromosome([[0, 34], [0, 3]], npcSize,
                             len(prevPopPool[0][0].scenario[0]))
