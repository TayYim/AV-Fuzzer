...
def isHitEdge(ego, sim, init_degree):
    ...
    diagnal_length = pow(ego.bounding_box.size.z, 2) + \
        pow(ego.bounding_box.size.x, 2)
...
def debugPos(ego, npc):
    ...
    print(" ^^^^^^^^ Ego Rotation: " + str(egoRotation) +
          ", NPC rotation: " + str(npcRotation) + " ^^^^^^^")
...
def findDistance(ego, npc):
    ...
    npc_y = npc.state.transform.position.y
    npc_z = npc.state.transform.position.z
    dis = math.pow(npc_x - ego_x, 2) + math.pow(npc_y -
                                                ego_y, 2) + math.pow(npc_z - ego_z, 2)
    dis = math.sqrt(dis)
    return dis
...
def isEgoFault(ego, npc, sim, init_degree):
    ...
    if isCrossed == True:
        if ego_x + 4 < npc_x and (npc.state.rotation.y < 271 and npc.state.rotation.y > 269):
            ...
            print(
                " --- Ego cross, side collision or front colision to NPC, EGO FAULT --- ")
            return True
    else:
