...
class Chromosome:
    def __init__(self, bounds, NPC_size, time_size):
        self.y = 0
        self.scenario = [[[] for i in range(time_size)]
                         for j in range(NPC_size)]  # This scenario
        self.bounds = bounds
        self.code_x1_length = NPC_size
        self.code_x2_length = time_size
        # in seconds, timeout timer for simulator execution per each scenario simulation
    ...
    def fix_init(self):
        for i in range(self.code_x1_length):        # For every NPC
            for j in range(self.code_x2_length):    # For every time slice
                # random.uniform(self.bounds[0][0], self.bounds[0][1])        # Init velocity
                v = (self.bounds[0][0] + self.bounds[0][1]) / float(2)
                # Keep straight #random.randrange(self.bounds[1][0], self.bounds[1][1])      # Init action
                a = 3
    ...
    def rand_init(self):
        for i in range(self.code_x1_length):        # For every NPC
            for j in range(self.code_x2_length):    # For every time slice
                # Init velocity
                v = random.uniform(self.bounds[0][0], self.bounds[0][1])
                a = random.randrange(
                    self.bounds[1][0], self.bounds[1][1])      # Init action
                self.scenario[i][j].append(v)
                self.scenario[i][j].append(a)
    ...
    def decoding(self):
        ...
        for x in range(0, 100):
            ...
            if resultObj != None and resultObj['fitness'] != '':
                ...
                util.print_debug(
                    " ***** " + str(x) + "th/10 trial: Fail to get fitness, try again ***** ")
