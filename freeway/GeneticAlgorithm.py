...
class GeneticAlgorithm:
    def __init__(self, bounds, pm, pc, pop_size, NPC_size, time_size, max_gen):
        ...
        self.bestIndex = 0
        self.g_best = None
        # Checkpoint path, if set, GE will start from the checkpoint (population object)
        self.ck_path = None
        # Record which chromosomes have been touched in each generation
        self.touched_chs = []

        # Set flag for local iterative search (LIS)
        self.isInLis = False
    ...
    def ga(self):
        ...
        for i in range(self.max_gen):                       # i th generation.
            ...
            # Find the scenario with the best fitness score in current generation
            best, bestIndex = self.find_best()
            ...
            util.print_debug(
                " ###### Best score of the generation: " + str(best.y) + " ###### ")
            if self.g_best.y < best.y:                  # Record the best fitness score across all generations
                self.g_best = copy.deepcopy(best)

            util.print_debug(" ###### Best score overall: " +
                             str(self.g_best.y) + " ###### ")

            N_generation = self.pop
            # Record the scenario with the best score over all generations
            N_b = self.g_best
            ...
            self.take_checkpoint(
                N_generation, 'generation-' + str(i) + '-at-' + date_time)

            #################### Start the Restart Process ###################
            if noprogress == True and not self.isInLis:
                util.print_debug(
                    " ###### Restart Based on Generation: " + str(i) + " ###### ")
                oldCkName = 'GaCheckpoints'
                newPop = generateRestart.generateRestart(
                    oldCkName, 1000, self.bounds)
            ...
            if os.path.exists("GaCheckpoints") == True:
                ...
                for eachChs in self.pop:
                    eachSimilarity = generateRestart.getSimularityOfScenarioVsPrevPop(
                        eachChs, prePopPool)
                    simiSum += eachSimilarity
                util.print_debug(
                    " ==== Similarity compared with all prior generations: " + str(simiSum/float(self.pop_size)))

            # Log fitness etc
            f = open('Progress.log', 'a')
            f.write(str(i) + " " + str(best.y) + " " + str(self.g_best.y) +
                    " " + str(simiSum/float(self.pop_size)) + " " + date_time + "\n")
            f.close()

            if best.y > self.bestYAfterRestart:
                self.bestYAfterRestart = best.y
                # Only allow one level of recursion
                if i > (self.lastRestartGen + self.minLisGen) and self.isInLis == False:
                    ################## Start LIS #################
                    util.print_debug(
                        " \n\n === Start of Local Iterative Search === \n\n")
                    # Increase mutation rate a little bit to jump out of local maxima
                    lis = GeneticAlgorithm(self.bounds, (self.pm * 1.5), self.pc,
                                           self.pop_size, self.NPC_size, self.time_size, self.numOfGenInLis)
                    lis.setLisPop(self.g_best)
                    lis.setLisFlag()
                    lisBestChs = lis.ga()
                    util.print_debug(
                        " --- Best fitness in LIS: " + str(lisBestChs.y))
                    if lisBestChs.y > self.g_best.y:
                        # Let's replace this
                        self.pop[bestIndex] = copy.deepcopy(lisBestChs)
                        util.print_debug(" --- Find better scenario in LIS: LIS->" +
                                         str(lisBestChs.y) + ", original->" + str(self.g_best.y))
                    else:
                        util.print_debug(
                            " --- LIS does not find any better scenarios")
                    util.print_debug(
    ...
    def cross(self):
        ...
        for i in range(int(self.pop_size / 2.0)):
            ...
            if self.pc > random.random():
                ...
                pop_j.scenario[swap_index] = copy.deepcopy(
                    pop_i.scenario[swap_index])
    ...
    def mutation(self, gen):
        ...
        while(i < len(self.pop)):
            ...
            if self.pm >= random.random():
                ...
                if actionIndex == 0:
                    # Change Speed
                    eachChs.scenario[npc_index][time_index][0] = random.uniform(
                        self.bounds[0][0], self.bounds[0][1])
                elif actionIndex == 1:
                    # Change direction
                    eachChs.scenario[npc_index][time_index][1] = random.randrange(
            ...
            if eachChs in self.touched_chs:
                ...
                util.print_debug(
                    " --- The chromosome has not been touched in this generation, skip simulation. ---")

            util.print_debug(
                " --- In mutation: Current scenario has y = " + str(eachChs.y))
    ...
    def select_top2(self):
        ...
        for i in range(0, self.pop_size):
            util.print_debug(
                " === Fitness result of the scenario is " + str(self.pop[i].y) + " === ")
            ...
            if self.pop[i].y == maxFitness:
                for j in range(int(self.pop_size / 2.0)):
                    selectedChromosome = Chromosome(
                        self.bounds, self.NPC_size, self.time_size)
